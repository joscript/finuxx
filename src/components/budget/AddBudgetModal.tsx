  ...
  const fetchCategories = async () => {
    ...
    try {
      ...
      if (response.success && response.data?.categories) {
        const mapped: AvailableCategory[] = response.data.categories.map(
          (cat: Category) => ({
            name: cat.name,
            ...
            {step === "category" ? (
                  ...
                  {isFetchingCategories ? (
                    ...
                    <View className="flex-row flex-wrap">
                      {availableToAdd.map((category, index) => (
                        ...
                        >
                          <View
                            className={`${category.iconBgColor} w-12 h-12 rounded-xl items-center justify-center mb-3`}
                          >
                            <Ionicons
              ...
              <>
                ...
                <View className="flex-row items-center mb-6">
                  <View
                    className={`${selectedCategory?.iconBgColor} w-14 h-14 rounded-2xl items-center justify-center`}
                  >
                    <Ionicons
